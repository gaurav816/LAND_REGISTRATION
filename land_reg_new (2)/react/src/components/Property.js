...
const Property = (props) => {
    ...
    const getProperty = async () =>{
      ...
      for(let i=0; i<_totalIndices; i++){
        ...
        if(ownerOwns[3].words[0] != 0){
            const landDetails = await contract.extractPropertyDetails(ownerOwns[0], ownerOwns[1], ownerOwns[2], ownerOwns[3].words[0], {
              from: account
            })
  ...
  const markAvailableFunction = async (indx) =>{
      await contract.markAvailable(indx, {from: account});
      setReload(!reload);
      console.log(indx);
  }
  ...
  return (
    <div className='container' style={{marginBottom: '2rem'}}>
        ...
        <div className="no-result-div">
          <h1 className='no-result text-center pt-5 font-weight-bold'>NO PROPERTIES FOUND</h1>
        </div>
        :
