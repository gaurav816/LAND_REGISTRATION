...
const Requests = (props) => {
  ...
  useEffect(() => {
    const getRequests = async () =>{
      ...
      for(let i=0; i<_totalIndices; i++){
        const ownerOwns = await contract.extractOwnerOwned(i, {from: account});  // returns object
        
        // if survey no. != 0
        if(ownerOwns[3].words[0] != 0){

          const reqCnt_propId = await contract.extractRequestCount(ownerOwns[0], ownerOwns[1], ownerOwns[2], ownerOwns[3].words[0],{
            from: account
          })
          ...
          if(noOfRequests > 0){
            ...
            for(let j = 0; j<noOfRequests; j++){
              ...
              const requester = await contract.extractRequester(ownerOwns[0], ownerOwns[1], ownerOwns[2], ownerOwns[3].words[0], j, {
                from: account
              })
  ...
  const handleAcceptReq = async (_index, _reqNo) => {
      await contract.RequestAccept(_index, _reqNo, {from: account});
      setReload(!reload);
  }
    ...
    <div className='container'>
      {  
        ...
        <div className="no-result-div">
          <h1 className='no-result text-center pt-5 font-weight-bold'>NO INCOMING REQUESTS</h1>
        </div>
        :
